...
export default function LandingPage() {
  return (
    <div className="min-h-screen bg-white flex flex-col">
      ...
      {/* Hero */}
      <main className="flex-1 flex flex-col items-center justify-center px-4 py-14 text-center sm:px-6 md:py-24">
        ...
        {/* Headline */}
        <h1 className="text-4xl font-bold text-gray-900 max-w-2xl leading-tight md:text-5xl">
        ...
        {/* CTA buttons */}
        <div className="mt-8 flex w-full max-w-sm flex-col items-stretch gap-3 md:w-auto md:max-w-none md:flex-row md:items-center">
          <Link to="/signup" className="btn-primary btn-lg w-full md:w-auto">
            Start free
          </Link>
          <Link to="/login" className="btn-secondary btn-lg w-full md:w-auto">
            Log in
          </Link>
        </div>

        {/* Terminal demo */}
        <div className="mt-12 w-full max-w-xl text-left md:mt-14">
          {/* Window chrome */}
          ...
          <div className="bg-gray-950 rounded-b-lg p-3 space-y-3 overflow-x-auto md:p-5">
            {/* Step 1: install */}
            <TerminalLine step="1" prompt="$" command={INSTALL_CMD} comment="install once" />
            {/* Step 2: run */}
            <TerminalLine step="2" prompt="$" command={TUNNEL_CMD} />
            {/* Output */}
            <div className="pt-1 min-w-max space-y-1 font-mono text-xs md:text-sm">
      ...
      {/* How it works */}
      <section className="border-t border-gray-100 px-4 py-14 sm:px-6 md:py-20">
...
function TerminalLine({ prompt, command, comment }) {
  return (
    <div className="group flex min-w-max flex-col gap-2 md:flex-row md:items-start md:gap-3">
      ...
      <CopyButton
        text={command}
        label="Copy"
        className="w-fit shrink-0 opacity-100 transition-opacity !border-gray-700 !text-gray-400 hover:!bg-gray-800 hover:!text-gray-200 md:opacity-0 md:group-hover:opacity-100"
      />
    </div>
  )
}
